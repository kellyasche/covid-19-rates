...
cases.100k.county.map <- ggplot(case.county.centroid.map) +
  ...
  geom_point_interactive(aes(x = X, y = Y, size = cases.100k, color = cases.100k, data_id = County, tooltip = paste(County, "\nAs of: ", Date, "\nTotal cases: ", comma(Cases, accuracy = 1.0), "\nTotal population: ", comma(total.pop, accuracy = 1.0), "\nCases per 100,000 population: ", comma(cases.100k, accuracy = 0.01), sep = "")), alpha = .9) +
